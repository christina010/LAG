...
class UnreachHeading(BaseTerminationCondition):
    ...
    def __init__(self, config):
        ...
        self.max_roll_increment =aircraft_config['max_roll_increment']
        ...
        self.roll_mod=aircraft_config['roll_mod']
    ...
    def get_termination(self, task, env, agent_id, info={}):
        ...
        if env.agents[agent_id].get_property_value(c.simulation_sim_time_sec) >= check_time:
            if math.fabs(env.agents[agent_id].get_property_value(c.delta_roll)) > 15:
                done = True
            # if current target heading is reached, random generate a new target heading
                ...
                delta_roll = env.np_random.uniform(-delta, delta) * self.max_roll_increment  #degree
                ...
                new_roll = env.agents[agent_id].get_property_value(c.target_roll_deg) + delta_roll
                new_roll = (new_roll -self.roll_mod) % (self.roll_mod*2)-self.roll_mod # mod 操作保证范围
                ...
                env.agents[agent_id].set_property_value(c.target_roll_deg, new_roll)
